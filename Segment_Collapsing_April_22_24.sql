...
,CASE WHEN ( CASE
	...
	Abs(( (Lead(ImputedAADT ,1) OVER (partition BY LRSID ,highwayclasscode ORDER BY [BeginLogMile], endLogmile) - ImputedAADT) / CONVERT(FLOAT ,ImputedAADT))) END ) < .025 
...
,CASE WHEN ( CASE
	WHEN isnull(nullif(ImputedAADT,0),0) = 0 THEN 0  ELSE 
	Abs(( (Lag(ImputedAADT ,1) OVER (partition BY LRSID ,highwayclasscode ORDER BY [BeginLogMile], endLogmile) - ImputedAADT) / CONVERT(FLOAT ,ImputedAADT))) END ) < .025 
